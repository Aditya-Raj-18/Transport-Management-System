...
function tripDateTime(trip) {
  ...
  const date = new Date(`${trip.date}T${trip.time || "23:59"}`);
  return Number.isNaN(date.getTime()) ? null : date;
}

function isTripExpired(trip) {
  const travelAt = tripDateTime(trip);
  return travelAt ? travelAt < new Date() : false;
}

function refreshCompletedBookings() {
  let changed = false;
...
function availableBookingTrips() {
  ...
  return state.trips.filter((trip) => trip.date === bookingDate && trip.status !== "Completed" && !isTripExpired(trip) && availableSeats(trip) > 0);
}
...
function filteredRoutes(date = null) {
  ...
  const routes = date
    ? state.routes.filter((route) => state.trips.some((trip) => trip.routeId === route.id && trip.date === date && !isTripExpired(trip) && availableSeats(trip) > 0))
    : state.routes;
...
function routeCard(route) {
  const routeTrips = state.trips.filter((trip) => trip.routeId === route.id && (!bookingDate || trip.date === bookingDate) && !isTripExpired(trip) && availableSeats(trip) > 0);
  return `
    <div class="list-item">
...
render();
setInterval(() => {
  if (refreshCompletedBookings()) render();
}, 60000);
